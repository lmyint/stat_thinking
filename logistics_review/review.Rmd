...
With the distribution function, we can calculate the mean:
...
par(mar = c(5,4,0.1,2))
barplot(dbinom(0:10, size = 10, prob = 0.3), xlab = "x", ylab = "p(x)")
```
...
\log \left( \frac{E[Y_i]}{1 - E[Y_i]} \right) &= \beta_0 + \beta_1 x_{i1} + \cdots + \beta_p x_{ip}
\end{align*}
$$
...
**Interaction terms** represent a difference in slope between different groups of people. Also called **effect modification**

## Regression: coefficient hypothesis testing

The null hypothesis $H_0: \beta_j = 0$ indicates that covariate $j$ has no effect on the outcome.
...
Set an acceptable type I error rate
...
## Regression: coefficient hypothesis testing {.smaller}
