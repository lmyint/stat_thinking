...
$$ E[\mathrm{June\:weight\:under\:control}] = \mathrm{Weight}_\mathrm{Sept} $$
...
$$ E[\mathrm{June\:weight\:under\:control}] = \beta_0 + \beta_1\mathrm{sex} + \beta_3\mathrm{Weight}_\mathrm{Sept} $$

------------------------------------------------------------------------------
 Units   Sex, Sept. weight   $Y_i(0)$                       $Y_i(1)$   $T_i$
------- ------------------- ------------------------------ ---------- -------
  1             M,140        $\beta_0+\beta_1+140\beta_3$     141        1

  2             F,120        $\beta_0+120\beta_3$             119        1

  .               .              .                             .         .

  .               .              .                             .         .

  N             M,150        $\beta_0+\beta_1+150\beta_3$     144        1
------------------------------------------------------------------------------
...
## How well did the randomization work? Randomization designs
...
Example: key points from Zika article on reviewing evidence for causation

Put this slide towards the beginning before RCM
...
```{r}
## Add time trend for control group before and after intervention
## This trend does not have a jump at intervention time
```

## Interrupted time series: caveats

- History: other events that co-occur with the intervention
    - Less of a problem if have multiple communities, or some comparison communities
- Testing: frequent assessment may affect the measurements themselves
    - Having many baseline observations helps; also helps if see change in outcome right at point of intervention
...
- Regression to the mean: tendency of extreme scores to regress toward the mean on later measurement occasions
    - Helps if have many repeated measures
...
- Lack of control of the implementation of the program: hard to separate out effects of the process of implementing the program from effects of the program itself
    - Ideal would be to choose the time program is implemented randomly
