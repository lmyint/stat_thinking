...
Statistician A computes the weight change from September to May for each freshman and separates these differences by sex. He compares these weight changes in males and females using a t-test. He fails to reject the null hypothesis of no weight difference and concludes that there is no evidence that the cafeteria is causing differential weight gain in male and female freshmen.

Statistician B models May weight as a function of September weight and gender. Specifically, he fits the following regression model:
...
What made that last example tricky for the two statisticians? We know that the university did not provide any information about student weight outcomes in the absence of the cafeteria, but this was not immediately obvious to the statisticians.
...
An assignment mechanism is a deterministic or probabilitic process that determines which treatment each unit receives. We will see shortly that the idea of an assignment mechanism generalizes the idea of selection bias.
...
## Extrapolation as a special case of generalizability considerations
